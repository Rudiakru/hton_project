...
def _compute_validation_summary(
    *,
    source: str,
    moments_by_match: dict[str, list[object]],
    labels: dict,
) -> dict:
    """Compute a tiny, pre-labeled validation summary.

    This is build-time only (shipped as frozen JSON in the pack) and is meant to
    be directional, not statistically meaningful.
    """

    k = int(labels.get("k") or 3)
    label_map: dict[str, list[str]] = labels.get("labels") or {}
    per_match = []
    matched_total = 0
    labeled_total = 0

    for match_id, labeled_ids in sorted(label_map.items()):
        predicted = [getattr(m, "moment_id", None) for m in (moments_by_match.get(match_id) or [])[:k]]
        predicted = [p for p in predicted if p]
        labeled_set = set(labeled_ids)
        hits = [mid for mid in predicted if mid in labeled_set]
    ...
    return {
        ...
        "labeled_matches": len(per_match),
        "labeled_total": labeled_total,
        "matched_total": matched_total,
        "sample_note": labels.get("sample_note"),
        "per_match": per_match,
    }
...
def main() -> int:
    ...
    # Validation (tiny manual label set; shipped as frozen JSON)
    labels_path = REPO_ROOT / "data" / "validation_labels.json"
    if labels_path.exists():
        labels = json.loads(labels_path.read_text(encoding="utf-8"))
        summary = _compute_validation_summary(source=source, moments_by_match=moments_by_match, labels=labels)
        _write_json(out_root / "processed" / "validation_labels.json", labels)
        _write_json(out_root / "processed" / "validation_summary.json", summary)
    else:
        _write_json(
            out_root / "processed" / "validation_summary.json",
            {
                "status": "missing",
                "note": "No validation labels shipped in this build.",
                "fix": "Add data/validation_labels.json and rebuild the demo pack.",
            },
        )
