...
const DemoDashboard = () => {
  // Start in loading=true so fast clicks during the initial React effect don't
  // hit `startDemo` before `matches/teams` are populated (stage-safety hardening).
  ...
  const dataReady = matches.length > 0 && teams.length > 0;
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const [health, m, t, integ, validationSummary, mask, bench] = await Promise.all([
          api.demoHealth(),
          api.demoMatches(),
          api.demoTeams(),
          api.demoIntegrity(),
  ...
  const startDemo = async () => {
    ...
    if (!targetMatch || !targetTeam) {
      setError('Demo data is still loading. Fix: wait 1–2 seconds for the offline pack to load, then click Start Demo again.');
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4 text-sm" data-testid="validation-panel">
          <div className="font-semibold mb-2">Validation (tiny, manual, directional)</div>
          {!validation ? (
            <div className="opacity-70">Loading validation…</div>
          ) : (validation.status === 'missing') ? (
            <div className="opacity-80">
              No validation summary shipped in this pack. Fix: rebuild the demo pack.
            </div>
          ) : (
            <>
              <div className="opacity-90">
                On <span className="font-mono">{validation.labeled_matches}</span> labeled matches, top-
                <span className="font-mono">{validation.k}</span> moments matched{' '}
                <span className="font-mono">{validation.matched_total}</span>/
                <span className="font-mono">{validation.labeled_total}</span>.
              </div>
              <div className="text-xs text-slate-300 mt-2">
                {validation.sample_note || 'Small sample; meant to be honest and directional, not a benchmark.'}
              </div>
            </>
          )}
        </div>
            ...
            {!integrity ? (
              ...
              <div className="text-sm space-y-2">
                ...
                <div className="text-xs text-slate-300">
                  Offline demo pack only (no internet, no runtime LLM calls). Scope is strictly the extracted `DEMO_PACK_ROOT`.
                </div>
                ...
                <div className="text-sm text-slate-300">
                  Match count:{' '}
                  <span data-testid="match-count" className="text-cyan-200 font-mono">{integrity.match_count}</span>
                </div>
                <div className="text-sm text-slate-300">
                  Total events:{' '}
                  <span data-testid="total-events" className="text-cyan-200 font-mono">{integrity.total_events}</span>
                  {' '}| Total moments:{' '}
                  <span data-testid="total-moments" className="text-cyan-200 font-mono">{integrity.total_moments}</span>
                </div>

                {showMethodology && (
                  <div className="text-xs font-mono opacity-80 space-y-1">
                    <div>total_patterns: {integrity.total_patterns}</div>
                    <div>total_evidence_panels: {integrity.total_evidence_panels}</div>
                  </div>
                )}
              </div>
            )}
